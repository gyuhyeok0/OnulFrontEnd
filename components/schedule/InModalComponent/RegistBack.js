...
const RegistBack = () => {
    ...
    const { myExercises: myBack } = useSelector((state) => state.backExercises || {});
    ...
    useEffect(() => {
        if (Array.isArray(myBack) && myBack.length > 0) {
            const exerciseIds = myBack.map(exercise => exercise.id);
    ...
    const filteredExercises = useMemo(() => {
        return exercises
            ...
            .filter((exercise) => exercise.mainMuscleGroup === "등")
            .filter((exercise) => {
    ...
    useEffect(() => {
        if (searchQuery.trim() !== '') {
            ...
            if (foundExercise.mainMuscleGroup !== "등") {
                setSearchMessage(
                    t('exerciseMessage', {
            ...
            <View style={styles.contentWrapper}>
                <Animated.View style={[styles.myExercise, { height: heightAnimation }]}>
                    ...
                    <View style={styles.myMainExercise}>
                     <Text style={styles.myExerciseText}>{t('registModal.myExerciseSchedule', { bodyParts: t('bodyParts.등') })}</Text>
                    {!isCollapsed && (
                                    ...
                                    {scheduleExercises.length > 0 ? (
                                        <View style={styles.exerciseGrid}>
                                            {scheduleExercises.map((exerciseId) => {
                                                const exercise = exercises.find(e => e.id === exerciseId && e.mainMuscleGroup === "등");
                                                return (
